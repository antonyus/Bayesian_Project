...
# Filter for 2011–2019
vacc_data_q6 <- vacc_data %>% filter(Birth.Year >= 2011 & Birth.Year <= 2019)

# Build Y and N matrices
Y <- matrix(vacc_data_q6$Vaccinated, nrow = length(unique(vacc_data_q6$Geography)), byrow = TRUE)
N <- matrix(vacc_data_q6$Sample.Size, nrow = length(unique(vacc_data_q6$Geography)), byrow = TRUE)

bugs_data_q6 <- list(
  Y = Y,
  N = N,
  BirthYear = as.numeric(as.factor(vacc_data_q6$Birth.Year)),
  N_region = length(unique(vacc_data_q6$Geography)),
  N_year = length(unique(vacc_data_q6$Birth.Year))
)
...
mcmc_samples_Q6 <- as.mcmc(regression_Q6)

# Combine chains into one matrix
samples_matrix <- do.call(rbind, mcmc_samples_Q6)

# Get all beta1[i] columns (i.e., slopes per location)
beta1_columns <- grep("^beta1\\[", colnames(samples_matrix))
beta1_samples <- samples_matrix[, beta1_columns]

# Compute posterior probability that each slope is > 0
...
posterior_prob_df <- data.frame(
  Region = region_names,
  Posterior_Prob_Increase = round(posterior_probs, 4)
)
...
index_2019 <- length(unique(vacc_data_q6$Birth.Year))  # index for 2019
...
# Storage for all pairwise ratio results
all_ratios_df <- data.frame()

# Loop through all unique pairs
for (i in 1:(num_regions - 1)) {
  for (j in (i + 1):num_regions) {
    
    # Predicted coverage for both regions in 2019
    ...
    # Ratio of coverage A to B
    ratio_samples <- pi_A / pi_B

    # Add to results
    all_ratios_df <- rbind(all_ratios_df, data.frame(
...
all_ratios_df %>%
  knitr::kable(
    format = "html",
    booktabs = TRUE,
    caption = "Pairwise comparisons of vaccination coverage ratios in 2019"
  ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
...
```{r Plotting Pairwise Ratios with 95% Credible Intervals echo=FALSE}

# Labels for each comparison
all_ratios_df$Comparison <- paste0(all_ratios_df$Region_A, " / ", all_ratios_df$Region_B)

# Plot
ggplot(all_ratios_df, aes(x = reorder(Comparison, -Mean_Ratio), y = Mean_Ratio)) +
  geom_point(size = 3, color = "#007C8A") +
  geom_errorbar(aes(ymin = Lower_95, ymax = Upper_95), width = 0.2, color = "#007C8A") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray40") +
  labs(
    title = "Pairwise Coverage Ratios in 2019",
    subtitle = "Mean with 95% credible intervals (Region A / Region B)",
    x = "Region Comparison",
    y = "Coverage Ratio"
  ) +
  coord_flip() +
