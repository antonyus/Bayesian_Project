...
The code structure is provided below. First we specify the model structure. For each region and year cohort we ask to calculate the binomial likelihood by using a loop function (see chunk below). 
...
The we specify the logistic function. As it can be seen below we did not indexed the beta coefficients. This way only one intercept and one beta coefficient for the effect of year of birth will be calculated for all regions. 

```{r eval=FALSE, echo=TRUE}
logit(pi[i, j]) <- beta0 + beta1 * BirthYear[j] # same beta0, beta1 for all regions
```

The a non informative prior is specified. Since we a re working with the logit of the vaccination rate we cannot use a beta distribution as in question one. The support for the logit of the vaccination rate is infinite. Therefore we use a normal distribution centered around zero but with very high variance. In BUGS language this means low precision, hence the code below.
...
The rest of the code specifies the matrix to be used as data input and finally the model run. We used a burn in of 500 (meaning that the first 500 samples are discarded), thinning equal to 2 (meaning only every other sample are retained), and then three chains are run.
...
Looking at the trace plots below, for both \(\beta_0\) and \(\beta_1\) we can see that the chains jump around the same general range and visit different areas of the parameter space. No drift is apparent. All chains seem to oscillate around a similar range of values. A visual check favours a good convergence of the model. 
