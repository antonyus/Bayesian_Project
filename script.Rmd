...
# Model structure assuming one intercept and slop for all regions
model_structure <- " 
model {
  for (i in 1:N_region) {  # Loop over regions
  for (j in 1:N_year) {  # Loop over years (cohorts)
...
## Answer
The posterior density for \(\beta_0\) should captures the baseline vaccination coverage when year of birth is zero. Therefore, here it does not have a direct interpretation in terms of vaccination coverage, but it is still necessary for the model. Looking at \(\beta_1\)posterior density we see that is centered very close to 0, and its mass spans both positive and negative values. This means that the effect of year of birth is limited and there is high uncertainty regarding its estimate. Posterior summary measures for \(\beta_0\) and \(\beta_1\) are provided in table 3. If we then apply the inverse logit transformation to the linear predictor we can obtain the estimated vaccination coverage per year by using as \(\beta_0\) and \(\beta_1\) their mean. Data are reported in the table 4 below and indeed reflect an approximate vaccination around 90%. 
...
To evaluate whether trends in vaccination coverage differ across regions, we use a hierarchical logistic regression model. The number of vaccinated children in each region and birth year is assumed to follow a Binomial distribution with region-specific probabilities.
...
```{r Model Implementation in JAGS, echo=FALSE, message=FALSE, warning=FALSE}
writeLines(model_structure, "logistic_model_Q6.bug")

# Filter and arrange data
vacc_data_q6 <- vacc_data %>%
  filter(Birth.Year >= 2011 & Birth.Year <= 2019) %>%
  arrange(Geography, Birth.Year)

# Define region and year levels
region_levels <- unique(vacc_data_q6$Geography)
year_levels <- sort(unique(vacc_data_q6$Birth.Year))

# Pivot Vaccinated and Sample Size into matrices with consistent indexing
vacc_matrix <- vacc_data_q6 %>%
...
params_q6 <- c("beta0", "beta1")

# Run JAGS
regression_Q6 <- jags(
  data = bugs_data_q6,
  parameters.to.save = params_q6,
  model.file = "logistic_model_Q6.bug",
  n.chains = 3,
  n.iter = 5000,
  n.burnin = 500,
  n.thin = 2
...
summary_Q6_clean <- as.data.frame(regression_Q6$BUGSoutput$summary)
summary_Q6_clean$variable <- rownames(summary_Q6_clean)

# Extract posterior means
beta0_means <- summary_Q6_clean %>%
  filter(grepl("^beta0\\[", variable)) %>%
  pull(mean)

beta1_means <- summary_Q6_clean %>%
...
posterior_pi %>%
  knitr::kable(
    ...
    col.names = c("Region", "Year", "Estimated Coverage (percent)")
  ) %>%
...
samples_matrix <- do.call(rbind, as.mcmc(regression_Q6))
...
# Get region names
region_names <- unique(vacc_data_q6$Geography)
...
regions <- unique(vacc_data_q6$Geography)
years <- sort(unique(vacc_data_q6$Birth.Year))
...
# Calculate observed vaccination proportion
observed <- vacc_data_q6 %>%
  group_by(Geography, Birth.Year) %>%
  summarise(Observed = sum(Vaccinated) / sum(Sample.Size)) %>%
  rename(Region = Geography, Year = Birth.Year)

# Merge with posterior summary
plot_data <- left_join(posterior_summary, observed, by = c("Region", "Year"))
```

The following plot shows the estimated vaccination coverage by region and birth year, along with 95% credible intervals for the model estimates. The shaded regions represent uncertainty around the predicted coverage, while observed vaccination proportions are overlaid as white points with outlines. If observed data points fall within the shaded bands, this indicates that the model fits the data well. Differences in slope and uncertainty width across regions reflect regional variation in trends and data size.

```{r Plot, echo=FALSE, fig.cap="Estimated vaccination coverage by region and birth year and observed data (dots). "}
ggplot(plot_data, aes(x = Year, y = Mean, color = Region, fill = Region)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.2, linetype = "dashed") +
  geom_point(aes(y = Observed), shape = 21, fill = "white", size = 2, stroke = 1.2) +
  facet_wrap(~ Region) +
  scale_x_continuous(breaks = seq(min(plot_data$Year), max(plot_data$Year), by = 1)) +  # Interval every 1 years
  labs(
    title = "Vaccination Coverage by Region and Year",
    y = "Vaccination Coverage (Estimated with 95% CI)",
    x = "Birth Year"
  ) +
  theme_minimal()+
...
```markdown
I think there is something strange in this plot because the expected coverage 
(mean) is lower than in the previews table and much lower than th observations 
which makes it strange since we used a vague prior right? mm...no I tried and I 
get the same results. I was not expecting such a low mean for mississippi 
actually. And also isent the cred interval unexpectedly wide? I think it might 
depend on the fact that our predicted variable is bounded 0-1 and some samnples
are so close to zero that produce distorted results? Like absurdely samal or large
probabilities?
```

```{r alternative plot, echo=FALSE}
observed_coverage <- vacc_data_q6 %>%
  group_by(Geography, Birth.Year) %>%
  summarise(Observed = sum(Vaccinated) / sum(Sample.Size)) %>%
  rename(Region = Geography, Year = Birth.Year)

predicted_coverage <- posterior_pi
predicted_coverage$Estimated_Coverage_Percent <- predicted_coverage$Estimated_Coverage_Percent / 100

merged_data <- left_join((observed_coverage), predicted_coverage, by = c("Region", "Year"))

samples_matrix <- regression_Q6$BUGSoutput$sims.list
n_iter <- dim(samples_matrix$beta0)[1]       # e.g. 6750 iterations
n_regions <- length(region_levels)           # e.g. 3
n_years <- length(year_levels)               # e.g. 9

estimated_pi_array <- array(NA, dim = c(n_iter, n_regions, n_years))

for (iter in 1:n_iter) {
  for (i in 1:n_regions) {
    for (j in 1:n_years) {
      eta <- samples_matrix$beta0[iter, i] + samples_matrix$beta1[iter, i] * year_index[j]
      estimated_pi_array[iter, i, j] <- 1 / (1 + exp(-eta))  # inverse logit
    }
  }
}

credible_interval <- map_dfr(1:n_regions, function(i) {
  map_dfr(1:n_years, function(j) {
    sims <- estimated_pi_array[, i, j]
    tibble(
...
merged_data_full <- left_join(merged_data, credible_interval, by = c("Region", "Year"))


# Create a version of the dataset with an indicator for predicted points
predicted_points <- merged_data_full %>%
  mutate(PointType = "Predicted")

# Create a version with observed points (plotted as separate layer)
observed_points <- merged_data_full %>%
  mutate(PointType = "Observed",
         Estimated_Coverage_Percent = Observed)

# Combine both for plotting points
points_combined <- bind_rows(predicted_points, observed_points)

# Final ggplot
ggplot() +
  # Credible interval ribbon
  geom_ribbon(data = predicted_points,
              aes(x = Year, ymin = Lower, ymax = Upper, fill = Region),
              alpha = 0.2) +
  ...
  # Optional: customize colors (you can choose your own palette)
...
for (i in seq_along(regions)) {
  ...
  logit_pi <- beta0_samples[, i] + beta1_samples[, i] * length(unique(vacc_data_q6$Birth.Year)) + 1  # 2020 index (after 2011–2019)
  pi <- 1 / (1 + exp(-logit_pi))
...
Sorry what does Pred_Mean stands for here? The predicted vacc cover for 2020 
taking the mean beta for region and year effect from the samples?

```
## Interpretation

Table 9 compares the observed number of vaccinated children in 2020 with the predictive distributions derived from earlier years. For each region, the observed counts fall within the 95% predictive intervals. Specifically, Georgia observed 165 vaccinations, which is above the posterior mean (119.2) but still within the upper bound of the predictive interval (188). Mississippi and Wisconsin also reported observed counts (161 and 156, respectively) that exceed their respective predictive means (32.1 and 130.8), yet remain within the upper bounds of their prediction intervals. These results suggest that while the observed values were higher than expected, particularly in Mississippi, they do not fall outside the plausible range defined by the model. Therefore, there is no strong statistical evidence to suggest that vaccination numbers in 2020 were inconsistent with historical trends, although the magnitude of the deviations (especially in Mississippi) may warrant further investigation. Nevertheless, the upper bound of the 95% predictive intervals correspond to the total sample size for that year and the lower bound to zero. A critical observation is that because the predicted probabilities are bounded between 0 and 1, the impact of predictor variables is asymmetric depending on the value of the predicted probability. At extreme probabilities, the logistic curve is steep, meaning small changes in predictors cause large shifts in the predicted probabilities, which can exaggerate the impact of those predictors. This can create over-fitting in regions with sparse data (near 0 or 1), leading to predictions that are too sensitive to small changes. We handle this issue in the next question. 
...
```{r Pairwise Comparison Full and Trimmed, echo=FALSE}
# Prepare
region_names <- unique(vacc_data_q6$Geography)
num_regions <- length(region_names)
index_2019 <- length(unique(vacc_data_q6$Birth.Year))
...
```{r Table Trimmed, echo=FALSE}

posterior_ratios_trimmed <- posterior_ratios_df %>%
  filter(Ratio > 0.5, Ratio < 2)

summary_trimmed <- posterior_ratios_trimmed %>%
  group_by(Comparison) %>%
  summarise(
    Mean_Ratio = round(mean(Ratio), 3),
    Lower_95 = round(quantile(Ratio, 0.025), 3),
    Upper_95 = round(quantile(Ratio, 0.975), 3)
  )
...
summary_trimmed %>%
  knitr::kable(
    format = "latex",
    booktabs = TRUE,
    caption = "Trimmed posterior comparisons of vaccination coverage ratios in 2019 (ratios limited to 0.5–2)",
    col.names = c("Comparison", "Mean Ratio", "Lower 95  CI", "Upper 95 CI")
  ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```

```{r log ratios, echo=FALSE}
# Storage for log-ratio summaries
summary_logratios <- data.frame()
...
for (i in 1:(num_regions - 1)) {
  for (j in (i + 1):num_regions) {
    ...
    pi_A <- 1 / (1 + exp(-logit_A))
    pi_B <- 1 / (1 + exp(-logit_B))
    log_ratio_samples <- log(pi_A) - log(pi_B)

    comparison_label <- paste0(region_names[i], " / ", region_names[j])

    # Append to summary
    summary_logratios <- rbind(summary_logratios, data.frame(
      Comparison = comparison_label,
      Mean_Log_Ratio = round(mean(log_ratio_samples), 3),
      Lower_95 = round(quantile(log_ratio_samples, 0.025), 3),
      Upper_95 = round(quantile(log_ratio_samples, 0.975), 3)
    ))
  }
}
...
summary_logratios %>%
  knitr::kable(
    format = "latex",
    booktabs = TRUE,
    caption = "Log ratios of posterior comparisons of vaccination coverage ratios in 2019",
    col.names = c("Comparison", "Mean Ratio", "Lower 95  CI", "Upper 95 CI")
  ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
...
```{r densities, echo=FALSE, fig.cap="Posterior densities of vaccination coverage per region in year 2019."}
samples_matrix <- do.call(rbind, as.mcmc(regression_Q6))

# Extract beta0 and beta1 samples for each region
beta0_samples <- samples_matrix[, grep("^beta0\\[", colnames(samples_matrix))]
beta1_samples <- samples_matrix[, grep("^beta1\\[", colnames(samples_matrix))]

# Set BirthYear for 2019
year_2019 <- 2019

# Initialize matrix for predicted probabilities (coverage) for 2019
n_samples <- nrow(beta0_samples)  # Number of MCMC samples
n_regions <- ncol(beta0_samples)  # Number of regions

predicted_pi_2019 <- matrix(NA, nrow = n_samples, ncol = n_regions)

# Calculate inverse logit for 2019 for each region
for (i in 1:n_regions) {
  # Compute logit(pi) for 2019 for each posterior sample
  logit_pi_2019 <- beta0_samples[, i] + beta1_samples[, i] * year_2019
  
  # Apply inverse logit to calculate pi (vaccination probability)
  predicted_pi_2019[, i] <- 1 / (1 + exp(-logit_pi_2019))
}

# Check the first few rows of predicted probabilities for 2019
head(predicted_pi_2019)

# Convert predicted probabilities to a data frame for ggplot
predicted_pi_2019_df <- as.data.frame(predicted_pi_2019)
colnames(predicted_pi_2019_df) <- region_levels  # Set region names as column names

# Gather data for ggplot
predicted_pi_2019_long <- tidyr::pivot_longer(predicted_pi_2019_df, cols = everything(), names_to = "Region", values_to = "PredictedProbability")

# Plot the posterior distributions for 2019 probabilities
ggplot(predicted_pi_2019_long, aes(x = PredictedProbability, fill = Region)) +
  geom_density(alpha = 0.5) +
  labs(
    title = "Density of Predicted Vaccination Probabilities for 2019",
...
Table 10 presents the pairwise mean ratios of vaccination coverage across regions in 2019. Given that coverage values are constrained between 0 and 1, calculating ratios of posterior samples can result in extreme values when the denominator approaches zero. This issue arises even when the majority of posterior mass lies within a high-coverage range (e.g., 80–90%), as occasional draws near zero (e.g., 0.0001 or 0.001) can disproportionately inflate the resulting ratios. To address this, we apply two alternative strategies. First, in Table 11, we restrict the ratio distribution to a plausible range (e.g., 0.5–2.0), effectively trimming the influence of extreme outliers. Second, in Table 12, we present the distribution of log-transformed ratios, which better captures relative differences in a more stable and symmetric manner. Finally, the plot below provides posterior density to visually assess the degree of overlap in vaccination coverage between regions.

Visual inspection of the posterior distributions suggests that Georgia has a higher expected vaccination coverage than Mississippi, which in turn appears to exceed that of Wisconsin. Nonetheless, when examining trimmed posterior ratios, the mean values for all state pairs cluster near 1.0, and the corresponding 95% credible intervals include 1. This indicates that, although point estimates imply a potential ordering (Georgia > Mississippi > Wisconsin), the uncertainty is substantial enough that we cannot confidently reject the hypothesis of equal coverage among these states. 